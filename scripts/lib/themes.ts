...
export function translateTheme(entry: ThemeEntry, mode: ThemeMode): ThemeVariables {
    ...
    const accent = first(
        colors,
        ["primary.background", "accent.background", "ring"],
        DEFAULT_ACCENT,
    )
    ...
    return {
        ...
        "--dl-surface-alt": first(
            colors,
            ["list.hover.background", "muted.background", "secondary.background"],
            bg,
        ),
        "--dl-fg": fg,
        "--dl-muted": first(
            colors,
            ["muted.foreground", "tab.foreground", "popover.foreground"],
            fg,
        ),
